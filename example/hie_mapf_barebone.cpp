...
//        }
    ...
    // update goals to make the environment usable by the local agents
    void updateGoals(const std::vector<Location>& goals){
        m_goals = std::move(goals);
    }
...
//**********************************************************************
// Hie_MPAF starts from here
//**********************************************************************

struct LocalAgent{
    LocalAgent(int region_id) : region_id(region_id){}
...
int getRegionID(int start_x, int start_y, int goal_x, int goal_y, int dimx, int dimy, int seg){
    int start_id;
    int goal_id;

    start_id = int(start_x / (dimx / seg)) * seg + int(start_y / (dimy / seg) + 1);
    goal_id =  int(goal_x / (dimx / seg)) * seg + int(goal_y / (dimy / seg) + 1);

    if (start_id == goal_id) return start_id;
    else return 0;
}

void agentCluster(const std::vector<Location>& goals, const std::vector<State>& startStates,
                  std::vector<LocalAgent>& local_agents, GlobalAgent& global_agents, int dimx, int dimy, int seg){
    int region_id;
    for (size_t i = 0; i != goals.size(); ++i){
        region_id = getRegionID(startStates[i].x, startStates[i].y,
...
//    }
*/
    Environment global_map(dimx, dimy, obstacles, global_agents.goals, highways);
    std::cout << "The size of the goals is: " << global_agents.goals.size() << std::endl;
    HieMAPF<State, Action, int, Conflict, Constraints, Environment, Location> global_cbs(global_map);
    std::vector<PlanResult<State, Action, int>> global_path;

    bool global_success = global_cbs.search(global_agents.startStates, global_path);
    ...
    for (size_t i = 0; i < local_agents.size(); ++i){
        std::vector<PlanResult<State, Action, int>> local_path;
        global_cbs.updateMap(local_agents[i].goals);
        bool local_success = global_cbs.search(local_agents[i].startStates, local_path);
    }
