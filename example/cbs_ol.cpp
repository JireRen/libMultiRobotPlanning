...
//
std::vector<State> getCurrentState(std::vector<Location>& goals,
        const std::vector<PlanResult<State, Action, int>> solution, int time_step){
    std::vector<State> startStates;
    std::vector<Location> newGoals;

    for (size_t a = 0; a < solution.size(); ++a){
        if (solution[a].states.back().second >= time_step){
            startStates.emplace_back(State(0, solution[a].states[time_step].first.x,
                                           solution[a].states[time_step].first.y));
            newGoals.emplace_back(goals[a]);
        }
    }
    goals = newGoals;
    return startStates;
...
//
bool validateState(const std::vector<Location> goals,
        const std::unordered_set<Location> obstacles, const std::vector<State> startStates, int x, int y){
    return obstacles.find(Location(x, y)) == obstacles.end() &&
    std::find(goals.begin(), goals.end(), Location(x, y)) == goals.end() &&
    std::find(startStates.begin(), startStates.end(), State(0, x, y)) == startStates.end();
}

//
void generateOnlineAgents(std::vector<Location>& goals, const std::unordered_set<Location> obstacles,
        std::vector<State>& startStates, int max_num, int xlim, int ylim){
    int num_agents = rand() % max_num + 1;
    int start_x;
    int start_y;
    int goal_x;
    int goal_y;
    for (int i = 0; i < num_agents; ++i){
        ...
        goal_x = 1 + rand() % xlim;
        goal_y = 1 + rand() % ylim;
        while(!validateState(goals, obstacles, startStates, start_x, start_y)){
            start_x = 1 + rand() % xlim;
            start_y = 1 + rand() % ylim;
            std::cout << " trying to find new starts" << std::endl;
        }
        startStates.emplace_back(State(0, start_x, start_y));
        std::cout << " generate a new agent start at: [" << start_x << ", " << start_y << "] " << std::endl;
        while(!validateState(goals, obstacles, startStates, goal_x, goal_y)){
            goal_x = 1 + rand() % xlim;
            goal_y = 1 + rand() % ylim;
            std::cout << " trying to find new goals" << std::endl;
        }
        goals.emplace_back(Location(goal_x, goal_y));
        std::cout << " generate a new agent goal at: [" << goal_x << ", " << goal_y << "] " << std::endl;
    }
}
...
int main(int argc, char* argv[]){
    ...
    if (success) {
        ...
        std::cout << "The make span is : " << makespan << std::endl;
        ...
        for (int i = 0; i < makespan; ++i){
            if (randbool()){
                std::cout << " new agents coming at timestep: " << i << std::endl;
                startStates = getCurrentState(goals, solution, i);
                generateOnlineAgents(goals, obstacles, startStates, 3, dimx, dimy);

                std::cout << "The current size is : " << goals.size() << std::endl;

                Environment new_mapf(dimx, dimy, obstacles, goals);
                CBS<State, Action, int, Conflict, Constraints, Environment> new_cbs(new_mapf);
                solution.clear();
                bool new_success = new_cbs.search(startStates, solution);

                if (new_success) {
                    std::cout << "Online planning successful! " << std::endl;
                    out << "round" << counter << ":" << std::endl;
                    for (size_t a = 0; a < solution.size(); ++a){
                        out << "  agent" << a << ":" <<std::endl;
                        for (const auto& state: solution[a].states){
                            out << "      - x: " << state.first.x << std::endl
                ...
                else std::cout << "Online planning failed! " << std::endl;
            }
        }
    }
    return 0;
