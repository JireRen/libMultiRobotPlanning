 ...
 public:
  ...
  void initHeuristic(){
    ...
    for (size_t agentIdx = 0; agentIdx != m_goals.size(); ++agentIdx){
      ...
      m_highwayHeuristicTable[agentIdx].data[src_y][src_x] = 0;
      for (size_t i = 0; i != m_vertices.size(); ++i){
        for (size_t e = 0; e != m_edges.size(); ++e){
          // dist[from] + e < dist[to]
          if (m_highwayHeuristicTable[agentIdx].data[m_edges[e].fromy][m_edges[e].fromx]
              + m_edges[e].weight < m_highwayHeuristicTable[agentIdx].data[m_edges[e].toy][m_edges[e].tox]){
            m_highwayHeuristicTable[agentIdx].data[m_edges[e].toy][m_edges[e].tox]
                = m_edges[e].weight + m_highwayHeuristicTable[agentIdx].data[m_edges[e].fromy][m_edges[e].fromx];
          }
        }
      }
    }
